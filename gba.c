...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  UNUSED(color);
   for(int curRow = 0; curRow < height; curRow++){
    DMA[3].dst = &videoBuffer[OFFSET(row+curRow, col, 240)];
    DMA[3].src = &color;
    DMA[3].cnt = (width) | DMA_16 | DMA_SOURCE_FIXED |
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for(int curRow = 0; curRow < height; curRow++){
    DMA[3].dst = &videoBuffer[OFFSET(row+curRow, col, 240)];
    DMA[3].src = &image[OFFSET(curRow,0,width)];
    DMA[3].cnt = (width) | DMA_16 | DMA_SOURCE_INCREMENT |
     DMA_DESTINATION_INCREMENT | DMA_ON;
  }
}
